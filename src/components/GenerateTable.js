...
const generateBody = (data, text, filters) => {
  ...
  filters.forEach((x) => {
    firstFilter = numericFilters(firstFilter, x);
  });
  return firstFilter
      ...
      <tbody key={values.name} data-testid="planetsBody">
        <tr>
          {Object.values(values).map(
            (box, index) =>
              index !== 9 && (
                <td className="tableData" data-testid="Planet" key={box}>
                  {box}
                </td>
              )
          )}
...
const GenerateTable = () => {
  const { data, error, filters, text, formatString } = useContext(SWContext);
          ...
          <thead>
            <tr>
              {Object.keys(data[0]).map(
                (item) =>
                  item !== "residents" && (
                    <th className="tableHeader" key={`planet_${item}`}>
                      {formatString(item)}
                    </th>
                  )
              )}
